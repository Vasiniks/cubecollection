...
line(`  with a non-US source ON A PRODUCT RECORD : ${active.length - noNonUS.length}   <- not the same as "has a non-US source"; see below`);
...
line('');
line('  THE NON-US COLUMN UNDERSTATES THE ARCHIVE, AND THE NUMBER IS LEFT ALONE ON PURPOSE.');
line('  This table reads FAMILY, MODEL and VARIANT records only. Measured 2026-09-19: TWELVE');
line('  manufacturers hold a non-US source cited on their MANUFACTURER record and invisible here');
line('  — cubestyle, diansheng, fanxin, guojia, lanlan, lefun, mefferts, mf8, mfjs, particula,');
line('  x-man-design, xinlexin. Eight of the twelve predate 2026-09-19, so this is long-standing');
line('  and not an artefact of that day\'s additions. Several are FIRST-PARTY (dianshengtoys');
line('  about page, qiyitoys company history, mefferts official site) — the strongest evidence');
line('  there is, and this column cannot see it.');
line('  WHY THE NUMBER DID NOT MOVE. 3.6a check 2 does not ask for "a non-US source". It asks to');
line('  "include at least one non-US/English RETAILER in discovery", because one retailer\'s');
line('  regional catalogue can hold an entity the rest of the method never sees. A corporate');
line('  about-page is not a discovery channel, so most of those twelve would NOT satisfy the');
line('  check as written even once visible. Raising the count by widening the scan would be');
line('  picking the reading that yields a better number. The COLUMN HEADING was wrong and is');
line('  fixed; the measurement is not.');
line('  OPEN FOR THE OWNER: whether 3.6a check 2 means a non-US RETAILER USED IN DISCOVERY (as');
line('  written, and as measured) or any non-US evidence. Those are different questions and the');
line('  archive currently answers only the first.');
